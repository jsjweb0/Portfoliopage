...
export function AboutPage() {
  ...
  // 최상위 zIndex 계산
  const topZ = useMemo(
    () => Math.max(0, ...windows.map((w) => w.zIndex)),
    [windows],
  );
  ...
  const getTopOpenWindowId = (excludeId: number) => {
    ...
    return open.reduce((top, w) => (w.zIndex > top.zIndex ? w : top)).id;
  };
      ...
      <div className="windowPanel flex flex-wrap gap-1">
        ...
        <div>
          <pre className="max-w-full h-[185px] whitespace-pre-line min-h-[209px]">
            <img src={assetPath('images/icon/png/note-3.png')} alt="" /> CSS
            전역 적용
            <br />
            import "../assets/css/***.css"; //
            <br />
            CSS import는 **JS 모듈 시스템 상 "전역으로 한 번만 로드되면 끝"**,
            <br />안 쓰더라도,
            <br />
            이미 이 CSS는 번들에 포함되어 전체 앱에 전역 적용되어버림.
          </pre>
        </div>
            ...
            <div className="window">
              <div className="title-bar">
                <div className="title-bar-text">
                  <img
                    src={assetPath('images/icon/png/magnifying_glass_4-0.png')}
                    className="mr-px"
                    alt=""
                  />
                  업데이트 확인
