...
export const projects: Project[] = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        content:
          "1. SoundCloud API 키 노출 문제\n프론트엔드에서 SoundCloud API를 직접 호출하면 API 키가 브라우저에 노출될 수 있었습니다. Netlify Functions를 프록시 서버처럼 사용해 API 키를 서버 환경변수로 관리했습니다. 프론트엔드는 /api/search, /api/resolve, /api/stream 엔드포인트만 호출하고, 실제 SoundCloud 요청은 서버리스 함수에서 처리하도록 분리했습니다. API 키를 클라이언트에 노출하지 않고 외부 API 연동 기능을 유지할 수 있었습니다.\n\n2. API 사용량 초과\n초기 화면에서 여러 플레이리스트와 트랙 정보를 한 번에 불러오면서 Netlify 무료 사용량이 빠르게 소모되는 문제가 있었습니다. 초기 로딩 플레이리스트 수를 줄이고, 검색 결과와 스트림 URL을 localStorage에 24시간 캐싱했습니다. Netlify Functions 응답에도 캐시 헤더를 추가해 반복 요청 부담을 줄였습니다. 사용자가 같은 검색어를 반복하거나 페이지를 새로고침해도 불필요한 API 호출이 줄어들도록 개선했습니다.\n\n3. 재생 상태 동기화\n전역 플레이어, 플레이리스트, 검색 결과 프리뷰 등 여러 진입 경로에서 재생 상태가 충돌하는 문제가 있었습니다. 현재 트랙, 재생 여부, 재생 시간, 재생목록을 Context로 관리하고, 오디오 제어 로직을 커스텀 훅으로 분리했습니다. 여러 컴포넌트에서 동일한 재생 상태를 공유할 수 있게 되었고, PlayerBar와 PlaylistPanel의 상태 동기화가 안정적으로 동작하도록 개선했습니다.\n\n4. SoundCloud 공식 API 제공 제한\nSoundCloud 공식 API 사용이 제한돼서 비공식 API를 활용했습니다. 그래서 API 구조가 바뀔 때마다 영향 범위를 줄이기 위해 호출 로직을 훅과 서버리스 함수로 분리했고, 응답이 느릴 때 화면이 비어 보이지 않도록 스켈레톤 UI를 넣었습니다.",
      },
      {
        type: "tech",
        title: "Technical Points",
        content:
          "• 음악 검색 및 SoundCloud 트랙 재생\n• 랜덤 플레이리스트 로딩 및 재생 / 일시정지 / 이전 곡 / 다음 곡 / 시크바 / 볼륨 제어\n• 플레이리스트 Drag & Drop 순서 편집\n• Firebase 이메일 로그인로그인 사용자 기준 트랙 좋아요 저장\n• 현재 트랙, 재생 여부, 재생 시간, 재생목록을 Context로 전역 관리\n• 모바일 하단 플레이어 및 플레이리스트 패널 지원\n• 다크모드 지원\n• 공지 게시판 CRUD, 댓글, 페이지네이션 구현",
        },
      {
        type: "result",
        title: "아쉬운 점 / 개선 예정",
        content:
          "• 댓글 데이터 구조를 글 번호 기준이 아닌 문서 ID 기준으로 리팩토링\n• 모바일 Drag & Drop과 터치 스크롤이 충돌하는 부분을 편집 모드 중심으로 개선\n• 좋아요한 트랙만 모아보는 페이지 추가\n• 플레이어 키보드 접근성 개선이 필요. 예: Space 재생/일시정지, 방향키 시크 이동",
      },
    ],
  },
  {
